...
class TimelineCanvas {
    ...
    constructor(data, dispatcher) {
        ...
        this.trackContexts = data.get('tracks').value.map(({ name, tracks }) => (
            new TrackPattern(name, ctx, ctxWrap, dom, tracks)
        ));
    }
    ...
    onDoubleClick(e) {
        ...
        if (this.overItem) {
            this.dispatcher.fire('timeline:keyframe', this.overItem.track, this.currentTime);
        }
    }
    ...
    _paint() {
        ...
        let { value: frameStart } = this.data.get('ui:scrollTime');
        ...
        units = scaledTime.getTick2Units();
        count = (this.width - LEFT_GUTTER + offsetUnits) / units;

        // marker lines - main
        for (let i = 0; i < count; i++) {
            ctx.strokeStyle = Theme.font;
            ctx.beginPath();
            x = i * units + LEFT_GUTTER - offsetUnits;
        ...
        const mul = scaledTime.tickMark3 / scaledTime.tickMark2;
        ...
        const txt = formatFriendlySeconds(currentTime);
        const textWidth = ctx.measureText(txt).width;

        const baseLine = MARKER_TRACK_HEIGHT - 5, half_rect = textWidth / 2 + 4;
        ...
        ctx.lineTo(x + 5, baseLine);
        ctx.lineTo(x + half_rect, baseLine);
        ctx.lineTo(x + half_rect, baseLine - 14);
        ctx.lineTo(x - half_rect, baseLine - 14);
        ctx.lineTo(x - half_rect, baseLine);
        ctx.lineTo(x - 5, baseLine);
        ctx.closePath();
